      ...
      function filteredData() {
        return this.data.filter(item => {
          if (this.isconfirm) {
            ...
            else if ((this.startJPY >0 && this.endJPY >=0) && (rateJPY < this.startJPY || rateJPY > this.endJPY)){
              return false
            }
          }
    ...
    const rowToSave = this.data.find(item => item === found);
      ...
      value: function render() {
        ...
        ">
            ...
            padding: 26px;
        ">
            <input id="select-all" type="checkbox" class="table-cell-input input-styled" ${this.isapproval ? "disabled" : ""}>    
            </div>
            <div>
                <p>Select All</p>
            </div>
            ...
            <tr>
              ${this.columns.map(column => y`
                  <th style="${(
                                ...
                                || 
                                 ((column.label != "Action" &&  column.label != "Planning date" && column.label != "Supplier Name" && column.label != "Currency Key" && column.label != "Amount in document currency") 
                                  && (this.issubmission) && (this.isapproval)) 
                                || 
